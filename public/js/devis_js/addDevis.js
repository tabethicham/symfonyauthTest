  $(document).ready(function(){  
       ...
       //$("#devis_modalitess label ").css({"display":"none"}) 
        ...
        $("#nextDevis").click( function(event){ 
            event.preventDefault();
            //alert("rrrrrrr");
            var form_data = $("#ajaxdevis form").serialize();  //$(this).serialize(); //
            var inpuut=$('#lignes_devis_deviss').val();
           // alert(inpuut)
            //condition sur cham le client si rempli $ahjaax sinon message message d'erreur
            $.ajax({  
                url: (inpuut.length==0)?'/agent/devis/new':'/agent/devis/'+inpuut+'/edit',  
                type:       'POST',   
                data:    form_data ,
                async:      true,  
        ...
        $("#lignes_devis_next").click(function(){
          ...
          $("#devis_delaiss").show(); 
           ...
           $("#nextDevis").css("display","none");
           $("#nextDevisFin").css("display","inline");
